...
def compute_distances_two_loops(x_train, x_test):
  ...
  for i in range(num_train):
    for j in range(num_test):
      dist = (x_train[i] - x_test[j]) ** 2
      dists[i][j] = torch.sum(dist)
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_one_loop(x_train, x_test):
  ...
  # Replace "pass" statement with your code
  for i in range(num_train):
    dist = (x_train[i] - x_test) ** 2
    dists[i] = torch.sum(dist, dim=(1,2,3))
  ##############################################################################
  #                             END OF YOUR CODE                               #
  ##############################################################################
  return dists
...
def compute_distances_no_loops(x_train, x_test):
  ...
  # torch.zero(num_train, num_test)로 fixed 시켜놓고 덧셈하니까 값이 달라지네,,, ㄷㄷ
  dists = -(2 * torch.matmul(x_train, x_test.T)) + torch.sum(x_test**2, dim=(1)) + torch.sum(x_train**2, dim=(1)).view(num_train, 1)
...
def predict_labels(dists, y_train, k=1):
  ...
  for i in range(num_test):
    ...
    val, ix = y_train[indices].unique(return_counts=True)
    y_pred[i] = val[ix.argmax()]
...
class KnnClassifier:
  ...
  def predict(self, x_test, k=1):
    ...
    y_test_pred = predict_labels(dists, self.y_train, k=self.k)
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return y_test_pred
...
def knn_cross_validate(x_train, y_train, num_folds=5, k_choices=None):
  ...
  for k in k_choices:
    ...
    for i in range(num_folds-1):
      c1 = KnnClassifier(x_train_folds[i], y_train_folds[i])
      acc = c1.check_accuracy(x_train_folds[num_folds-1], y_train_folds[num_folds-1], k=k)
      accs.append(acc)
...
def knn_get_best_k(k_to_accuracies):
  ...
  for i in k_to_accuracies:
    for j in range(len(k_to_accuracies[1])):
      if k_to_accuracies[i][j] >= acc:
        acc = k_to_accuracies[i][j]
        best_k = i
  ##############################################################################
  #                            END OF YOUR CODE                                #
  ##############################################################################
